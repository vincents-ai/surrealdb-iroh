...
async fn test_subscription_filter_with_db() {
	...
	let filter = SubscriptionFilter::new().with_namespace("ns1").with_database("db1");
...
async fn test_multi_namespace_db() {
	...
	for ns in &namespaces {
		for db in &databases {
			let change =
				Change::set(ns.clone(), db.clone(), Bytes::from("key"), Bytes::from("value"));
			total_changes += 1;
			assert_eq!(change.namespace, *ns);
			assert_eq!(change.database, *db);
		}
...
async fn test_large_value() {
	...
	assert_eq!(change.value.as_ref().map(|v| v.len()).unwrap_or(0), 100 * 1024);
}
