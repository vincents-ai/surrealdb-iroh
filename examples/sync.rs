...
pub async fn run_basic_example() -> Result<()> {
	...
	let endpoint_id = runner.endpoint_id().ok_or_else(|| anyhow!("No endpoint ID"))?;
	let ticket = runner.generate_ticket().await?;
...
impl StorageHook for MyStorageHook {
	fn on_change(&self, change: surrealdb_iroh::Change) {
		...
		self.change_count.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
	}
	...
	fn on_sync_complete(&self, peer_id: &iroh::EndpointId, changes_applied: usize) {
		info!("✅ Sync complete with peer {:?}, {} changes applied", peer_id, changes_applied);
	}
}
...
async fn main() -> Result<()> {
	...
	tracing_subscriber::registry()
		.with(tracing_subscriber::EnvFilter::new("info,surrealdb_iroh=debug"))
	...
	match args.get(1).map(|s| s.as_str()) {
		Some("connect") => {
			let ticket = args.get(2).ok_or_else(|| anyhow!("Usage: sync connect <ticket>"))?;
