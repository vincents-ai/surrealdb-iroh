...
mod tests {
    ...
    async fn test_batch_flush_on_size() {
        ...
        let batcher = Arc::new(ChangeBatcher::new(
            BatchConfig::new().with_max_batch_size(2),
            tx,
        ));
    ...
    async fn test_graceful_shutdown() {
        ...
        assert!(
            ...
            "shutdown timed out - task may be stuck"
        );
        assert!(
            !batcher.is_task_running(),
            "task should not be running after shutdown"
        );
    }
