...
impl FileChangeStore {
	...
	pub async fn compact(&self, keep_from_offset: u64) -> Result<u64> {
		...
		loop {
			...
			if record_number.is_multiple_of(10000) {
				writer.flush().map_err(|e| Error::Other(anyhow::anyhow!("flush error: {}", e)))?;
	...
	#[deprecated(since = "0.1.1", note = "Use automatic recovery in FileChangeStore::new()")]
		...
		if let Some(ref mut f) = *file {
			...
			if let Err(e) = f.sync_all() {
				...
				return Err(Error::Other(anyhow::anyhow!(
					"sync failed (disk full?): {} - data may not be durable", e
				)));
			}
			...
			if changes.len() >= self.inner.index_interval {
				let mut index = self.inner.index.write();
				let current_offset =
					self.inner.tail_offset.read().saturating_sub(changes.len() as u64);
				for (i, _) in changes.iter().enumerate().step_by(self.inner.index_interval) {
...
mod tests {
	...
	async fn test_file_store_basic() {
		...
		let store = FileChangeStore::new(dir.path().to_path_buf(), 10).await.unwrap();
	...
	async fn test_file_store_multiple_changes() {
		let dir = tempdir().unwrap();
		let store = FileChangeStore::new(dir.path().to_path_buf(), 5).await.unwrap();
	...
	fn test_base64_url_decode_integration() {
		...
		assert_eq!(data.as_slice(), decoded.as_slice(), "roundtrip should preserve data");
	...
	async fn test_crash_recovery_temp_files() {
		...
		assert!(!compacting_path.exists(), "compaction temp should be removed");
