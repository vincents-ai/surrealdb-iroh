...
impl SurrealDbProtocol {
	...
	pub async fn handle_connection(&self, conn: Connection) -> Result<()> {
		...
		let (mut send, mut recv) =
			conn.accept_bi().await.map_err(|e| Error::Connection(e.to_string()))?;

		// Update peer state
		self.sync_manager.update_peer_state(&peer_id, crate::sync::SyncState::Syncing);
		...
		loop {
			...
			if len as usize > MAX_MESSAGE_SIZE {
				...
				return Err(Error::Connection("message size exceeds maximum allowed".to_string()));
			}
			...
			send.write_all(&resp_len).await.map_err(|e| Error::Connection(e.to_string()))?;
			send.write_all(&encoded).await.map_err(|e| Error::Connection(e.to_string()))?;
		...
		// Signal end of data
		send.finish().map_err(|e| Error::ConnectionClosed(format!("{}", e)))?;

		// Wait for connection to close
		conn.closed().await;

		self.sync_manager.update_peer_state(&peer_id, crate::sync::SyncState::Synchronized);
	...
	pub async fn connect_and_sync(&self, addr: iroh::EndpointAddr) -> Result<()> {
		...
		let (mut send, mut recv) =
			conn.open_bi().await.map_err(|e| Error::Connection(e.to_string()))?;
		...
		send.write_all(&req_len).await.map_err(|e| Error::Connection(e.to_string()))?;
		send.write_all(&encoded).await.map_err(|e| Error::Connection(e.to_string()))?;
		...
		loop {
			...
			// Read message
			let mut msg = vec![0u8; len as usize];
			recv.read_exact(&mut msg).await.map_err(|e| Error::Connection(e.to_string()))?;
		...
		send.write_all(&[0u8; 4]).await.map_err(|e| Error::Connection(e.to_string()))?;
