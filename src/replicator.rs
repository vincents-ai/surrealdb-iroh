...
impl Replicator {
	...
	async fn build_endpoint(&self) -> Result<Endpoint> {
		...
		if let Some(addr) = self.inner.config.bind_addr {
			...
			builder.bind().await.map_err(|e| Error::Endpoint(format!("{}", e)))
		} else {
			Endpoint::bind(N0).await.map_err(|e| Error::Endpoint(format!("{}", e)))
		}
	}
	...
	pub fn endpoint_id(&self) -> Option<iroh::EndpointId> {
		self.inner.endpoint.read().as_ref().map(|e| e.secret_key().public())
	}
